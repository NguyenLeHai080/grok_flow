...
function readStoredLanguage(): string | null {
  ...
  try {
    return window.localStorage.getItem("grok2api:language-preference");
  } catch {
    return null;
  }
}
...
function storeLanguage(language: string): void {
  ...
  try {
    window.localStorage.setItem("grok2api:language-preference", language);
  } catch {
    // Language changes still apply when browser storage is unavailable.
  }
}

type LanguagePreference = "auto" | "en" | "zh-CN";
...
function resolveLanguage(preference: LanguagePreference): "en" | "zh-CN" {
  return preference === "auto" ? detectBrowserLanguage() : preference;
}

const languagePreference = normalizeLanguagePreference(readStoredLanguage());
const initialLanguage = resolveLanguage(languagePreference);
