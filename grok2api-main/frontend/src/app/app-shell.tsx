...
export function AppShell() {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" side="top" sideOffset={8} className="w-56 p-1.5">
          ...
          <DropdownMenuSub>
            ...
            <DropdownMenuSubContent>
              <DropdownMenuItem onClick={() => setLanguagePreference("auto")}><Monitor />Automatic</DropdownMenuItem>
              <DropdownMenuItem onClick={() => void i18n.changeLanguage("zh-CN")}>简体中文</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setLanguagePreference("en")}>English</DropdownMenuItem>
            </DropdownMenuSubContent>
          </DropdownMenuSub>
  ...
  return (
    <div className={cn("min-h-screen bg-background", embedded && "g2a-runtime-embedded")}>
        {!embedded ? <aside className="fixed inset-y-0 left-0 z-30 hidden h-screen w-[288px] flex-col overflow-hidden bg-sidebar px-4 py-6 lg:flex">
        ...
        <div className={cn("flex min-h-screen flex-col", !embedded && "lg:pl-[288px]")}>
          {!embedded ? <header className="flex h-12 items-center justify-between border-b px-4 lg:hidden">
          ...
          <main className={cn("mx-auto w-full flex-1", embedded ? "max-w-none px-6 py-8" : "max-w-[1280px] px-5 sm:px-8", !embedded && (isMediaWorkspace ? "pt-8 pb-0 lg:pt-20" : "py-8 lg:py-20"))}>
            <Outlet />
          </main>
          {!embedded && !isMediaWorkspace ? <SiteFooter /> : null}
        </div>
